...
export const ClaudeTitleOutro916: React.FC<ClaudeTitleOutro916Props> = ({ title, slug }) => {
  ...
  return (
    <AbsoluteFill style={{
      ...
      {/* Title line — auto-shrink long identifier-style titles so they fit portrait width */}
      {(() => {
        const availW = 1080 - PAD_X * 2;
        const CHAR_W = 0.45; // serif bold, letterSpacing -0.02em
        const longest = titleBody.split(/\s+/).reduce((m, w) => Math.max(m, w.length), 1);
        const idealByLongestWord = availW / (longest * CHAR_W);
        const fontSize = Math.min(TITLE_FONT, idealByLongestWord);
        return (
          <div style={{
            fontFamily: SERIF,
            fontWeight: 700,

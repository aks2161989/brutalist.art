...
export const claudeTitleOutroSchema = z.object({
  ...
  // handle: per-playlist channel override — defaults to the hardcoded @NikBearBrown
  // per OUTRO-LOCK.md. The Brutalist playlist for @HumanitariansAI sets this to
  // '@HumanitariansAI' via the beat sheet.
  handle: z.string().default(HANDLE),
});
...
export const ClaudeTitleOutro: React.FC<ClaudeTitleOutroProps> = ({ title, slug, mascotAnimation, handle }) => {
  ...
  return (
    <AbsoluteFill style={{
      ...
      {/* Handle — @NikBearBrown by default; @HumanitariansAI override for the
          Brutalist playlist per the isolated-toolkit playlist adaptation. */}
      <div style={{
        fontFamily: SERIF,
        fontSize: 52,
        color: fg,

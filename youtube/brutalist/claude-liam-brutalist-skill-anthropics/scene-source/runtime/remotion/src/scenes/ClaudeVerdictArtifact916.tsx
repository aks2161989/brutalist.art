...
export const ClaudeVerdictArtifact916: React.FC<ClaudeVerdictArtifact916Props> = ({
  ...
  // FILL LAW: card must occupy the safe area — earlier 916 layout only used ~40%
  // of the vertical canvas; force a card height of 78% of frame so ink coverage
  // clears the 55% underfill floor without straining line spacing.
  const CARD_H = height * 0.78;
  const PAD_H = width * 0.055;
  const PAD_V = height * 0.028;
  const FONT_TITLE = height * 0.024;
  const FONT_HEADING = height * 0.032;
  const FONT_LINE = height * 0.023;
  const LINE_GAP = height * 0.024;
  ...
  return (
    <AbsoluteFill style={{ background: '#F2F0E9', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{
        ...
        {/* Body */}
        <div style={{ padding: `${PAD_V * 1.4}px ${PAD_H}px ${PAD_V * 1.8}px`, flex: '1 1 auto', display: 'flex', flexDirection: 'column', justifyContent: 'flex-start' }}>
          {/* Heading */}
          <div style={{
            fontFamily: SERIF,

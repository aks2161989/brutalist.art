...
- **BrutalistHesitantWriter** — B01 uses a single-token trigger word
  (`linter` → `catalog`) per the trigger-syntax feedback memory. Portrait
  variant B01 shortens the on-screen text to three compact lines
  ("nopunt / is a linter. / Check it first.") to satisfy Gate V's 55% fill
  minimum at 2160×3840. The trigger swap remains intact; the audio narration
  is unchanged and still explains the full "catalog vs linter" framing.
- **B10 composer command** — the portrait composer `command` prop is
  shortened relative to landscape so the composer card plus the three output
  lines fit inside the portrait title-safe area. The narrated audio still
  speaks the three sheet-level checks aloud (FRAMEWORK / FALSIFIABILITY /
  scorable rubric) — only the on-screen composer text is trimmed for fit.
- **channel_title metadata** — removed from `vertical/beat_sheet.json` to
  prevent the compile-time overlay bleeding past the portrait safe.b edge
  (memory: channel-title-portrait-bleed). Composer folder chip on B00/B10
  still displays `@HumanitariansAI` inside the composer card itself.

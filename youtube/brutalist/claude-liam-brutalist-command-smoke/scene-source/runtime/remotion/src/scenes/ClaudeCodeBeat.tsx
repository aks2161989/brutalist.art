  ...
  return (
    <AbsoluteFill style={{ background: CLAUDE.PAGE }}>
      ...
      <div style={{
        ...
        {/* Code lines — adaptive font size so long snippets stay inside the card */}
        {(() => null)()}
        <pre style={(() => {
          const baseFont = height * 0.022;
          const cardHeight = height * (1 - 0.08 - 0.18);
          const cardWidth = width * (1 - 0.07 - 0.07);
          const titleBarH = 44;
          const padY = 40;
          const padX = 52;
          const availH = cardHeight - titleBarH - padY;
          const availW = cardWidth - padX;
          const lineH = 1.72;
          const idealFontByHeight = availH / (Math.max(lines.length, 1) * lineH);
          const longest = lines.reduce((m, l) => Math.max(m, l.length), 1);
          const CHAR_WIDTH_RATIO = 0.6; // monospace ~0.6em per glyph
          const idealFontByWidth = availW / (longest * CHAR_WIDTH_RATIO);
          const fontSize = Math.min(baseFont, idealFontByHeight, idealFontByWidth);
          return {
            margin: 0,
            padding: '20px 26px',
            fontFamily: MONO,

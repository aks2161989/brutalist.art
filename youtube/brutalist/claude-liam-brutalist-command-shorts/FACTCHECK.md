...
time of this rebuild (run_id `f2da2cc936694334b92c9a741e8f66e2`):
...
**Source change note:** Between the previous build and this one, the `art` dispatcher
gained new commands before `shorts`, shifting the case from lines 81-83 to **102-104**
and `vertical` from 84-86 to **105-107**. The `shorts.py` SHORTS LAW docstring rules
2/3/4 were also expanded. B02 and B03 were re-rendered to reflect the current source.

## Dispatcher

- `./art shorts <reel>` is a top-level case at `art:102-104` that runs
  `exec python3 "$ART_HOME/runtime/scripts/shorts.py" "$@"`. Verified:
  `grep -n "shorts)" toolkit/art` returns `102:  shorts)` immediately followed
  by the exec line.
- `./art vertical <reel>` at `art:105-107` runs the same script with the extra
...
- No aliases: the dispatcher `case` matches only the literal strings
  ...
  lookup further down and either shows a SKILL.md or errors out.

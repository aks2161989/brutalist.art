# PROMPTS.md — The your-turn Skill (Short)

Inherited from the parent reel — both composer beats (B00 cold-open ask
and B12 handoff YOUR TURN) are retained unchanged in this Short. Every
composer field renders exactly what `beat_sheet.json` B00/B12 declares in
`shot.remotion.props`; the description below is a fair summary. Both are
`ClaudeComposerAsk916` on the cream stage with the terracotta accent.

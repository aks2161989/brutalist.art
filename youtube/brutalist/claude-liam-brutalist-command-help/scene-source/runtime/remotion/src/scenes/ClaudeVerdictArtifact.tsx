...
export const ClaudeVerdictArtifact: React.FC<ClaudeVerdictArtifactProps> = ({
  ...
  // Fill the frame: 90% of width, no hard px cap so the card scales with the
  // rendered canvas (was Math.min(width*0.84, 1560) — that cap collapsed the
  // card to ~45% of the SAFE box on 4K and tripped Gate V underfill).
  const cardW = width * 0.90;
  const scale1920 = width / 1920;
    ...
    <AbsoluteFill style={{ background: '#F2F0E9', alignItems: 'center', justifyContent: 'center' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          padding: '28px 48px', display: 'flex', alignItems: 'center', gap: 16,
        }}>
          <span style={{ fontFamily: SERIF, fontSize: 40 * scale1920, color: CLAUDE.INK, fontWeight: 600 }}>
            {artifactTitle}
          </span>
        </div>

        {/* Artifact body */}
        <div style={{ padding: '60px 72px 66px' }}>
          {/* Heading */}
          <div style={{
            fontFamily: SERIF, fontSize: 68 * scale1920, fontWeight: 700, color: CLAUDE.INK,
            marginBottom: 44,
          ...
          {lines.map((line, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                <span style={{ fontFamily: SANS, fontSize: 42 * scale1920, color: CLAUDE.SPARK, flexShrink: 0, marginTop: 3, fontWeight: 700 }}>
                  {i + 1}.
                </span>
                <span style={{ fontFamily: SANS, fontSize: 42 * scale1920, color: CLAUDE.INK, lineHeight: 1.55 }}>
                  {line}
                </span>
              </div>
            );

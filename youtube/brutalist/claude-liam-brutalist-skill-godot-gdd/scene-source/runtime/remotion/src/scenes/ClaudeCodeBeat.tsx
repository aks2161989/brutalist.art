...
export const ClaudeCodeBeat: React.FC<ClaudeCodeBeatProps> = ({ title, code, sparkLine, language = 'python', largeText = false, brandLabel }) => {
  ...
  const baseFontSize = largeText ? height * 0.032 : height * 0.022;
  const cardContentW = (width * 0.86) - 52;
  const codeFontSize = portrait ? Math.min(baseFontSize, cardContentW / (maxLineLen * 0.62)) : baseFontSize;

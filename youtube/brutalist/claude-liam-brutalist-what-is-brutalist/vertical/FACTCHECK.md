# FACTCHECK.md — What Is Brutalist? · 9:16 portrait companion
...
**Portrait-specific claim delta (vs. landscape `../FACTCHECK.md`).** The
narration and all body-beat claims are identical — same Kokoro-generated
audio, same measured durations. The one on-screen text that changes between
aspects is **B01**: the landscape frame corrects "video-editor / AI-clips"
to "plain-text-recipe / your-film" (the widest line fits in the 1920 canvas),
while the portrait frame corrects "editor" to "recipe" (short lines that fit
the 1080 canvas). Both corrections carry the same misconception: the film is
not the editor, it is the text-recipe. The audio does not change — the writer
is heard typing on both aspects, and the narration argues the same claim.
All other rows below apply verbatim to the portrait companion.

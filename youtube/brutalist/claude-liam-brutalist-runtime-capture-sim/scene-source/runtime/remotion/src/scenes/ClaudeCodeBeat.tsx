...
export const ClaudeCodeBeat: React.FC<ClaudeCodeBeatProps> = ({ title, code, sparkLine }) => {
  ...
  // Adaptive font sizing — bound by both axes so long snippets fit
  // BOTH landscape 1920x1080 AND portrait 1080x1920 cards without overflow.
  const longestLine = lines.reduce((n, l) => Math.max(n, l.length), 1);
  const codeAreaW = width * (1 - 0.07 * 2) - 52;   // horizontal padding
  const codeAreaH = height * (1 - 0.18 - 0.08) - 90; // minus title bar + padding
  const baseFont = height * 0.022;
  const fontFromH = (codeAreaH / (lines.length || 1)) / 1.72;
  const fontFromW = codeAreaW / (longestLine * 0.60); // mono char advance ≈ 0.6em
  const codeFont = Math.max(10, Math.min(baseFont, fontFromH, fontFromW));

...
export const WalkerGodotSetup: React.FC<WalkerGodotSetupProps> = (p) => {
  ...
  if(p.mode==='download'){
    ...
    const compact=portrait&&p.labels.length>4;
    content=<div style={{...shell,overflow:'hidden',display:'flex',flexDirection:portrait?'column':'row',alignItems:'stretch',gap:compact?12:portrait?22:20}}>
      {p.labels.map((label,i)=><React.Fragment key={label}><div style={{...panel,flex:1,minWidth:0,padding:compact?'10px 18px':portrait?'25px 35px':p.labels.length>=5?'35px 12px':'35px 25px',display:'flex',flexDirection:'column',justifyContent:'center',opacity:reveal(i),outline:i===Math.min(active,p.labels.length-1)?`3px solid ${C.SEND}`:'none'}}>
        <div style={{fontFamily:F.ui,fontSize:compact?20:26,color:C.INK_SOFT,marginBottom:compact?8:24}}>{String(i+1).padStart(2,'0')}</div>
        <div data-audit={'flow-'+i} style={{fontFamily:F.serif,fontSize:compact?34:portrait?52:46,lineHeight:1.12,overflowWrap:'anywhere'}}>{label}</div>
        <div style={{fontFamily:F.mono,fontSize:compact?20:portrait?29:28,lineHeight:1.5,marginTop:compact?6:28,whiteSpace:'pre-line',overflowWrap:'anywhere'}}>{p.details[i]}</div>
      </div>{i<p.labels.length-1&&<div style={{alignSelf:'center',fontSize:compact?24:38,color:C.INK,opacity:reveal(i+1)}}>{portrait?'↓':'→'}</div>}</React.Fragment>)}
    </div>;
  }

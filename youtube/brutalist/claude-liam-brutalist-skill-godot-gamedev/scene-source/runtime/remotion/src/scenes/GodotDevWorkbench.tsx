...
export const GodotDevWorkbench:React.FC<GodotDevWorkbenchProps>=(p)=>{
 ...
 const code=<div style={{...panel,overflow:'hidden'}}><div style={{...heading,fontFamily:F.mono,fontSize:23,overflowWrap:'anywhere'}}>{p.path}</div>
   <div style={{padding:'18px 16px',fontFamily:F.mono,fontSize:portrait?24:23,lineHeight:1.3}}>{p.code.split('\n').map((line,i)=>{
 ...
 return <AbsoluteFill style={{background:C.PAGE,color:colors.text,fontFamily:F.ui}}>
  <div style={{position:'absolute',left:pad,right:pad,top:portrait?height*.12:height*.045,fontFamily:F.serif,fontSize:portrait?62:60,lineHeight:1.1,color:C.INK}}>{p.title}</div>
  ...
  <div style={{position:'absolute',right:pad,bottom:portrait?height*.065:height*.048,fontFamily:F.serif,fontSize:28,color:C.INK}}>@NikBearBrown</div>
 </AbsoluteFill>;
};

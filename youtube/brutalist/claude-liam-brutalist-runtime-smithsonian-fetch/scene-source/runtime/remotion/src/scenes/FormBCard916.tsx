...
export const FormBCard916: React.FC<FormBCard916Props> = ({ title, items, dark }) => {
  ...
  // subSz capped at 0.025× so multi-item stacks don't overflow the safe container
  const subSz   = Math.max(Math.ceil(height * 0.020), Math.round(height * 0.025));
  ...
  return (
    <AbsoluteFill style={{ background: bg }}>
      {/* ink-extent anchors — force bbox to span SAFE916 height (≥55% fill law) */}
      <div style={{ position: 'absolute', top: '5.5%', left: '50%', width: 4, height: 4, background: fg }} />
      <div style={{ position: 'absolute', bottom: '5.5%', left: '50%', width: 4, height: 4, background: fg }} />

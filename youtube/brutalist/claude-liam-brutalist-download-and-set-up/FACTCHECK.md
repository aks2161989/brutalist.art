...
| 5 | `skills/` holds the builder recipes. | B03 | `skills/make/` directory in the source snapshot. Count was removed from narration (rebuild run 92aed5ea) because the README and art script gained new skills (godot-walkthrough, riff) — the specific count was no longer stable across versions. |
...
- **Feature count.** Narration originally said "fifteen builder recipes"; updated to "builder recipes" (count removed) because the source grew new skills (godot-walkthrough, riff) making any specific count unstable.

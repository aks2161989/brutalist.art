...
SKILL.md was updated between the original build (run_id 5d2d09528cd0403dac54ed33498aef23) and this invocation. Changes:

1. **Frontmatter description simplified.** The frontmatter `description:` block was shortened from a detailed feature list to a two-sentence summary. This is cosmetic — no impact on any beat's factual claims.
2. **New hard rule: Executable evidence** (inserted before §House laws). Rule text: "Real code/data outranks a picture of code/data. Run safe examples, retain their outputs and render them clearly; do not turn them into pantry chores." This rule reinforces REBUILD LAW and SHOW-DON'T-TELL, both already covered in B05 (ILLUSTRATE LAW) and the episode's general approach. This episode demonstrates concept illustrations, not code file execution, so the rule does not create any gap in the existing beats.
3. **New hard rule: Math** (inserted before §House laws). Rule text governs structured math rendering when math appears. This episode contains no math — the rule is not applicable.

Assessment: no existing claim is contradicted or staled by the changes. No re-render required. The episode's FACTCHECK table above remains accurate.

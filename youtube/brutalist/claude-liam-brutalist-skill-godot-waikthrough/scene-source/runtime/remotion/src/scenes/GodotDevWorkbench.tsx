...
export const GodotDevWorkbench:React.FC<GodotDevWorkbenchProps>=(p)=>{
 ...
 return <AbsoluteFill style={{background:C.PAGE,color:colors.text,fontFamily:F.ui}}>
  <div style={{position:'absolute',left:pad,right:pad,top:portrait?height*.055:height*.045,fontFamily:F.serif,fontSize:portrait?62:60,lineHeight:1.1,color:C.INK}}>{p.title}</div>
  ...
  <div style={{position:'absolute',right:pad,bottom:portrait?height*.055:height*.048,fontFamily:F.serif,fontSize:28,color:C.INK}}>@NikBearBrown</div>
 </AbsoluteFill>;
};

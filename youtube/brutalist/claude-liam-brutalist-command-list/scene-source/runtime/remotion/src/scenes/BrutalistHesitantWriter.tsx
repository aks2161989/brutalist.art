...
function buildActs(p: BrutalistHesitantWriterProps): Act[] {
  ...
  for (const token of tokens) {
    ...
    if (ti !== -1 && replacements[ti]) {
      ...
      acts.push({t: 'pause', ms: 500 + pick(0, 250)});
...
export const BrutalistHesitantWriter: React.FC<BrutalistHesitantWriterProps> = (props) => {
  ...
  // (landscape) or a 1080x1920 design reference (portrait). Without the
  // portrait branch the scale collapses to 0.5625 on 9:16 and Gate V fails
  // as MAJOR underfill regardless of fontSize (see memory).

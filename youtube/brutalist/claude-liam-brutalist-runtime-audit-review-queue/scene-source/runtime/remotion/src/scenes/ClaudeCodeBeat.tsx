...
export const ClaudeCodeBeat: React.FC<ClaudeCodeBeatProps> = ({ title, code, sparkLine, language = 'python', largeText = false, brandLabel }) => {
  ...
  // Adaptive font: bound by height/lines (vertical fit) and width/longest-line (horizontal fit)
  // so code cards work at both landscape (1920×1080) and portrait (1080×1920).
  const cardW = width * (1 - 0.07 * 2);
  const longestLine = lines.reduce((m, l) => Math.max(m, l.length), 1);
  const baseFontPx = largeText ? height * 0.032 : height * 0.022;
  const cardH = height * (1 - 0.08 - 0.18);
  const maxByHeight = cardH / (lines.length * 1.72 + 2);
  const charWidthRatio = 0.60; // mono char width ≈ 60% of fontSize
  const maxByWidth = (cardW * 0.88) / (longestLine * charWidthRatio);
  const codeFontPx = Math.min(baseFontPx, maxByHeight, maxByWidth);
  ...
  return (
    <AbsoluteFill style={{ background: CLAUDE.PAGE }}>
      ...
      <div style={{
        ...
        {brandLabel && <span style={{fontFamily:SERIF,fontSize:28,color:CLAUDE.INK}}>{brandLabel}</span>}
      </div>

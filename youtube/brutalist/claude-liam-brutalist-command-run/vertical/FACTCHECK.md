...
## Portrait deltas (vertical only)

- **B01 text/triggerWords** — the portrait sibling uses
  `"art run\nrenders\nthe master\nvideo file."` with single-token
  triggers `renders → compiles`, `master → review`, and
  `fontSize 180 lineSpacing 2.6`. The 4-line stanza fills the
  9:16 SAFE box (Gate V FILL_MIN 0.55). The corrected sentence
  ("art run compiles the review video file.") remains the reel's
  actual claim.
- **B10 command + output** — shortened for portrait so the composer
  content fits inside SAFE916 without bottom edge-bleed. The
  fuller 4-step task remains intact on the landscape master.
- Both deltas were captured before every rebuild; no cached clip
  survived from a prior wording.

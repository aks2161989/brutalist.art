...
## Portrait-only B01 iteration

B01's `BrutalistHesitantWriter` props were re-tuned for portrait after Gate V
flagged an underfill on the shipped landscape props (fontSize=148 collapses to
2× in portrait scale → 296 px effective, which both bled past left/right safe
and produced a 21%–41% coverage vs. the 55% floor). Two-step fix:

1. `fontSize: 148 → 92` — text now fits within 2160-px canvas horizontally.
2. Split widest line at a word break (`re-cuts the fellows video.` →
   `re-cuts the / fellows video.`, five lines total) and set
   `fontSize: 92 → 118`, `lineSpacing: 1.2 → 2.2`, `charMs: 20 → 14`,
   `hesitateBetween: 3 → 1`, `hesitateWithin: 1 → 0`, `mistakeRate: 3 → 2`,
   `seed: fellows-bluf-vertical-e71dd104-v2`. Final coverage 56%, correction
   still lands mid-timeline.

Landscape B01 was NOT changed by this iteration.
